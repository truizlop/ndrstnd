...
ndrstnd analyzes with Codex, Claude Code, or Pi and uses the agent’s existing authenticated session. It never stores a token itself.

## Choosing the agent

Every command accepts `--agent codex`, `--agent claude`, or `--agent pi`:

    ndrstnd auth login --agent pi
    ndrstnd review feature/my-change --base main --agent pi

The same value can be set with `NDRSTND_AGENT=pi`. Without an explicit agent, ndrstnd uses the agent hosting the command when it runs inside Codex, Claude Code, or Pi (as it does when the installed skill triggers it); otherwise it falls back to the first installed CLI, preferring Codex. `ndrstnd auth status` reports all three agents, and `ndrstnd skill install` installs the skill for every agent that is set up on the machine.
...
- `src/server/analysis-core.ts` builds prompts and validates the analysis contract; `src/server/analyze.ts` runs the analysis through the selected agent, which `src/server/agent.ts` resolves and `src/server/codex.ts`, `src/server/claude.ts`, and `src/server/pi.ts` implement.
