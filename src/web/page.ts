...
export async function renderWorkspace(session: StoredReviewSession, revision: AnalysisRevision, launchToken: string): Promise<string> {
  return renderPage(session, revision, launchToken, false);
}

export async function renderArtifact(session: StoredReviewSession, revision: AnalysisRevision): Promise<string> {
  return renderPage(session, revision, "", true);
}

async function renderPage(session: StoredReviewSession, revision: AnalysisRevision, launchToken: string, artifact: boolean): Promise<string> {
...
<body><div class="app-shell">
  ...
  <section id="trailer" class="view active"><div class="story-tools"><p class="review-summary">${escapeHtml(revision.document.summary)}</p></div><p class="coverage">${session.input.files.length} files · ${session.input.hunks.length} evidence hunks · ${revision.document.chapters.length} story steps</p><div class="story-toolbar"><div id="map" class="map" hidden>${Object.entries(counts).map(([key, value]) => `<div><span class="dot ${key}"></span>${escapeHtml(key)} <strong>${value}</strong></div>`).join("")}</div><button id="collapse-all">Collapse all</button></div><div class="chapter-list">${renderChapters(revision.document, session.input.hunks, filePaths, session.input.files, highlighter)}</div>${renderOtherFilesChanged(session.input.files, session.input.hunks)}${renderOmitted(revision.document, session.input.hunks)}</section>
  ...
  <section id="diff" class="view"><p class="section-title">Every patch hunk</p>${session.input.files.map((file) => renderFullDiff(file, session.input.hunks.filter((hunk) => hunk.fileId === file.id), highlighter)).join("")}</section>
...
function renderEvidence(hunk: DiffHunk, path: string, highlighter: Highlighter): string {
  ...
  const excerpt = focused.map(({ line, index }) => {
    ...
    previousIndex = index;
    return `${omission}${renderEvidenceLine(line, resolveLanguage(path), highlighter)}`;
  }).join("");
  const omittedCount = hunk.lines.length - focused.length;
  return `<article class="evidence focused-evidence"><header><span>▱ ${escapeHtml(path)}</span><span>Focused excerpt · @@ −${hunk.oldStart} +${hunk.newStart} @@</span></header><pre>${excerpt}</pre>${omittedCount > 0 ? `<p class="evidence-context">${omittedCount} routine line${omittedCount === 1 ? "" : "s"} omitted</p>` : ""}</article>`;
}
...
const mapStyles = `
.breadcrumbs strong{color:#374151;font-weight:650}.chapter-list{transition:opacity 220ms ease,transform 220ms ease}.sidebar.collapsed nav{display:grid;grid-template-columns:1fr;justify-items:center;align-content:start}.sidebar.collapsed .nav-item{display:grid;place-items:center;width:42px;height:42px;min-height:42px;padding:0;line-height:1}.sidebar.collapsed .nav-item span{display:grid;place-items:center;width:20px;height:20px;font-size:16px;line-height:1;text-align:center;transform:none}.sidebar.collapsed .nav-item[data-view="tests"] span{font-size:14px;transform:translateY(-.5px)}.story-level-0 .chapter-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;max-height:none;overflow:visible;border:0;border-radius:0;opacity:1;transform:none;pointer-events:auto}.story-level-0 .chapter{border:1px solid #dfe5ea;border-radius:10px;overflow:hidden;background:#fff;box-shadow:0 1px 2px #172b4d0a;animation:map-card-in 240ms ease both}.story-level-0 .chapter-toggle{display:grid;grid-template-columns:34px minmax(0,1fr);grid-template-areas:"number copy";align-items:start;gap:12px;min-height:128px;padding:17px}.story-level-0 .chapter-number{grid-area:number}.story-level-0 .chapter-copy{grid-area:copy;gap:7px}.story-level-0 .chapter-copy strong{font-size:15px;line-height:1.35}.story-level-0 .chapter-copy small{display:none}.story-level-0 .chapter-tags{margin-top:2px}.story-level-0 .chevron{display:none}.story-level-0 .chapter-detail{display:none!important}.story-level-0 .chapter:hover{border-color:#b9d9f5;box-shadow:0 8px 18px #172b4d12;transform:translateY(-2px)}@keyframes map-card-in{from{opacity:0;transform:translateY(8px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}}@media(max-width:1120px){.story-level-0 .chapter-list{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:620px){.story-level-0 .chapter-list{grid-template-columns:1fr;gap:10px}.story-level-0 .chapter-toggle{min-height:112px}}@media(prefers-reduced-motion:reduce){.story-level-0 .chapter{animation:none;transition:none}}
`;
