...
<body><div class="app-shell">
  ...
  <aside class="inspector" aria-label="Review details"><header class="inspector-header"><h2>Review progress</h2><button class="collapse-inspector panel-toggle" aria-label="Collapse review details" aria-expanded="true">›</button></header><div class="inspector-content"><div class="progress"><div class="ring">${data.document.chapters.length}</div><div><strong>chapters</strong><span>ready to understand</span></div></div><section><h3>Focus areas</h3>${Object.entries(categoryCounts(data.document)).map(([category, count]) => `<div class="stat-row"><span>${riskIcon[category] ?? "•"} ${escapeHtml(category)}</span><strong>${count}</strong></div>`).join("")}</section><section><h3>Files</h3><div class="stat-row"><span>Changed files</span><strong>${data.files.length}</strong></div><div class="stat-row"><span>Evidence hunks</span><strong>${data.hunks.length}</strong></div></section><section><h3>Actions</h3><button class="inspector-action" data-action="export">⇧ Export review…</button><button class="inspector-action" data-action="print">Print review…</button><button class="inspector-action" data-action="copy-summary">Copy summary prompt</button></section></div></aside>
