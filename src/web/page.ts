...
function renderTestPlanExplanation(themes: TestThemeModel[]): string {
  return themes.map((theme) => `<section class="test-explanation-group"><h2>${escapeHtml(theme.title)}</h2>${theme.cases.length === 0 ? `<p class="empty-note">No associated test evidence</p>` : theme.cases.map((testCase) => `<article class="test-explanation-card" data-test-case="${escapeHtml(testCase.id)}"><h3>${escapeHtml(testCase.name)}</h3>${renderSemantic(testCase.chapter.before, testCase.chapter.after)}<dl><div><dt>What the test demonstrates</dt><dd>${escapeHtml(testCase.chapter.synopsis)}</dd></div><div><dt>Related theme</dt><dd>${escapeHtml(theme.title)}</dd></div>${theme.storyClaims[0] ? `<div><dt>Related story claim</dt><dd>${escapeHtml(theme.storyClaims[0].title)}</dd></div>` : ""}<div><dt>Associated test case</dt><dd>${escapeHtml(testCase.filePath)}</dd></div></dl></article>`).join("")}</section>`).join("");
}
...
const mapStyles = `
.breadcrumbs strong{color:#374151;font-weight:650}.chapter-list{transition:opacity 220ms ease,transform 220ms ease}.nav-icon svg{display:block;width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.sidebar.collapsed nav{display:grid;grid-template-columns:1fr;justify-items:center;align-content:start}.sidebar.collapsed .nav-item{display:grid;place-items:center;width:42px;height:42px;min-height:42px;padding:0;line-height:1}.sidebar.collapsed .nav-item .nav-icon{display:grid;place-items:center;width:20px;height:20px;line-height:1;text-align:center;transform:none}.sidebar.collapsed .nav-item .nav-icon svg{width:16px;height:16px}.chapter-map-meta,.chapter-churn-bar{display:none}.story-level-0 .chapter-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;max-height:none;overflow:visible;border:0;border-radius:0;opacity:1;transform:none;pointer-events:none}.story-level-0 .chapter{border:1px solid #dfe5ea;border-radius:10px;overflow:hidden;background:#fff;box-shadow:0 1px 2px #172b4d0a;animation:map-card-in 240ms ease both}.story-level-0 .chapter-toggle{display:grid;grid-template-columns:34px minmax(0,1fr);grid-template-areas:"number copy";align-items:start;gap:12px;min-height:128px;padding:17px;cursor:default}.story-level-0 .chapter-number{grid-area:number}.story-level-0 .chapter-copy{grid-area:copy;gap:7px;min-height:94px}.story-level-0 .chapter-copy strong{font-size:15px;line-height:1.35}.story-level-0 .chapter-copy small{display:none}.story-level-0 .chapter-tags{margin-top:2px}.story-level-0 .chapter-map-meta{display:flex;align-items:center;flex-wrap:wrap;gap:6px 9px;margin-top:auto;color:#6d7883;font-size:11px;line-height:1.2}.story-level-0 .chapter-churn{display:flex;gap:6px;font-weight:700}.story-level-0 .chapter-map-meta .additions{color:#168447}.story-level-0 .chapter-map-meta .deletions{color:#c84c43}.story-level-0 .chapter-churn-bar{display:flex;width:100%;height:5px;margin-top:3px;overflow:hidden;border-radius:999px;background:#edf1f4}.story-level-0 .chapter-churn-bar i{display:block}.story-level-0 .chapter-churn-bar .additions{width:calc(var(--add) * 1%);background:#8fd0a4}.story-level-0 .chapter-churn-bar .deletions{width:calc(var(--delete) * 1%);background:#efa19b}.story-level-0 .chevron{display:none}.story-level-0 .chapter-detail{display:none!important}.story-level-0 .chapter:hover{border-color:#dfe5ea;box-shadow:0 1px 2px #172b4d0a;transform:none}@keyframes map-card-in{from{opacity:0;transform:translateY(8px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}}@media(max-width:1120px){.story-level-0 .chapter-list{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:620px){.story-level-0 .chapter-list{grid-template-columns:1fr;gap:10px}.story-level-0 .chapter-toggle{min-height:112px}.story-level-0 .chapter-copy{min-height:78px}}@media(prefers-reduced-motion:reduce){.story-level-0 .chapter{animation:none;transition:none}}
`;
...
const testPlanStyles = `.test-plan-subtitle{margin:-6px 0 14px;color:#66727f;font-size:13px;line-height:1.45}.test-plan-summary{display:flex;flex-wrap:wrap;gap:7px;margin:0 0 16px}.test-plan-summary span,.test-inline-meta span,.test-theme-meta span,.test-state{display:inline-flex;align-items:center;min-height:24px;padding:4px 7px;border-radius:6px;background:#f3f5f7;color:#5d6975;font-size:11px;font-weight:560}.test-plan-level{display:none}.story-level-0 #tests .test-plan-map,.story-level-1 #tests .test-plan-summary-level,.story-level-2 #tests .test-plan-explanation,.story-level-3 #tests .test-plan-evidence,.story-level-4 #tests .test-plan-raw{display:block}.test-generated-summary{margin:0 0 12px;color:#4c5863;font-size:13px;line-height:1.45}.test-theme-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.test-theme-card{min-height:126px;border:1px solid #dfe5ea;border-radius:8px;background:#fff;padding:15px;box-shadow:0 1px 2px #172b4d0a}.test-theme-card h2,.test-behavior-group h2,.test-explanation-group h2,.test-raw-section>h2{margin:0 0 6px;color:#29323a;font-size:14px;font-weight:650}.test-theme-card p{margin:0;color:#68747f;font-size:12px;line-height:1.45}.test-theme-meta{display:flex;flex-wrap:wrap;gap:6px;margin-top:14px}.test-behavior-group,.test-explanation-group,.test-raw-section{border:1px solid #e2e6ea;border-radius:8px;background:#fff;margin:0 0 12px;padding:15px}.test-behavior{border-top:1px solid #edf0f2;padding:10px 0}.test-behavior:first-of-type{border-top:0}.test-behavior button{display:grid;gap:4px;width:100%;border:0;background:transparent;padding:0;text-align:left;cursor:pointer}.test-behavior button:hover strong{color:#167cd1}.test-behavior strong,.test-evidence-card summary strong{color:#2b333b;font-size:13px;font-weight:650}.test-behavior button span,.test-evidence-card summary small{color:#68747f;font-size:12px;line-height:1.45}.test-inline-meta{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.test-explanation-card{border-top:1px solid #edf0f2;padding:14px 0}.test-explanation-card:first-of-type{border-top:0}.test-explanation-card h3{margin:0 0 10px;color:#2b333b;font-size:13px}.test-explanation-card .semantic{margin:0 0 12px}.test-explanation-card dl{display:grid;gap:9px;margin:0}.test-explanation-card dl div{display:grid;grid-template-columns:160px minmax(0,1fr);gap:12px}.test-explanation-card dt{color:#596673;font-size:11px;font-weight:650}.test-explanation-card dd{margin:0;color:#5f6b76;font-size:12px;line-height:1.45}.test-evidence-card{border:1px solid #e2e6ea;border-radius:8px;background:#fff;margin:0 0 10px;overflow:hidden}.test-evidence-card summary{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;cursor:pointer;background:#fafbfc}.test-evidence-card summary>span:first-child{display:grid;gap:3px;min-width:0}.test-evidence-card .evidence{border-left:0;border-right:0;border-radius:0;margin:0}.test-evidence-meta{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;padding:12px 14px;border-top:1px solid #edf0f2}.test-evidence-meta div{display:grid;gap:3px}.test-evidence-meta strong{font-size:11px;color:#596673}.test-evidence-meta span{font-size:12px;color:#68747f}.test-raw-artifact{margin:10px 0 0}.test-raw-artifact h2{margin:0 0 8px;font-size:12px;color:#3d4751}.test-raw-section .empty-note{margin:4px 0 0}.empty-note{font-size:12px;color:#888;line-height:1.45}@media(max-width:980px){.test-theme-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.test-evidence-meta{grid-template-columns:1fr}}@media(max-width:620px){.test-theme-grid{grid-template-columns:1fr}.test-explanation-card dl div{grid-template-columns:1fr;gap:3px}.test-evidence-card summary{align-items:flex-start;flex-direction:column}.test-plan-summary span,.test-inline-meta span,.test-theme-meta span,.test-state{font-size:10px}}`;
...
(() => {
  ...
  setZoom(1);
  const setZoomControlsVisible = (view) => { const controls = document.querySelector('.story-zoom-controls'); if (controls) controls.hidden = view !== 'trailer' && view !== 'tests'; updateZoomLabel(Number(document.body.dataset.storyLevel ?? 1)); };
